...
if __name__ == '__main__':
    ms = torch.linspace(0.1, 2., 50)
    mag1 = torch.zeros(100,1)
    mag2 = torch.zeros(100,1)
    magd = torch.zeros(100,1)
    ang1 = torch.zeros(100,1)
    ang2 = torch.zeros(100,1)
    angd = torch.zeros(100,1)
    
    fig, ax = plt.subplots(1, 2)
    for m_current in ms:
        T = 100 # define terminal time for testing
        h = 6.626 * 1e-34 # planck constant
        m = torch.exp(m_current) * h # mass of electron
        cnst = h / (2 * m)
        ...
        mag1 = torch.cat((mag1,u1_mag.unsqueeze(-1)),dim=1)
        mag2 = torch.cat((mag2,u2_mag.unsqueeze(-1)),dim=1)
        magd = torch.cat((magd,udiff_mag.unsqueeze(-1)),dim=1)
        ang1 = torch.cat((ang1,u1_ang.unsqueeze(-1)),dim=1)
        ang2 = torch.cat((ang2,u2_ang.unsqueeze(-1)),dim=1)
        angd = torch.cat((angd,udiff_ang.unsqueeze(-1)),dim=1)
    
        ax[0].plot(u1_mag, label='Method 1', color='r', alpha = (torch.exp(m_current)/torch.exp(torch.max(ms))).item())
        #ax[0].plot(u2_mag, label='Method 2',color='slateblue', alpha = (torch.exp(m_current)/torch.exp(torch.max(ms))).item())
        #ax[0].plot(udiff_mag, label='Difference',color='tomato', alpha = (torch.exp(m_current)/torch.exp(torch.max(ms))).item())
        ax[0].set_ylabel('Magnitude')
        ax[0].set_xlabel('Time')
        ax[1].plot(u1_ang, color='r', alpha = (torch.exp(m_current)/torch.exp(torch.max(ms))).item())
        #ax[1].plot(u2_ang, color='slateblue', alpha = (torch.exp(m_current)/torch.exp(torch.max(ms))).item())
        #ax[1].plot(udiff_ang, color='tomato', alpha = (torch.exp(m_current)/torch.exp(torch.max(ms))).item())
