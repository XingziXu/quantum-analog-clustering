...
def graph_clustering_static(u, d_M, v_inds, num_itr, deflation, Nrows, case, base, base_dir, power, gnd_truth, start, node_num=[0]):
    ...
    for i in tqdm (range(start ,N), desc="Calculating clusters with DMD..."):
        ...
        if i in node_num:
            ...
            df_real.to_csv(base_dir + "\\canonical\\wave\\result\\output_"+base+"\\eigenvalues_"+case+".csv")
            ...
            plt.savefig(base_dir + "\\canonical\\wave\\figure\\plots_"+base+"\\modes_real_"+case+"_node"+str(i + 1)+".png")
            plt.close()
            ...
            plt.savefig(base_dir + "\\canonical\\wave\\figure\\plots_"+base+"\\modes_imag_"+case+"_node"+str(i + 1)+".png")
            plt.close()
            ...
            plt.savefig(base_dir + "\\canonical\\wave\\figure\\plots_"+base+"\\eigenvalues_"+case+"_node"+str(i + 1)+".png")
            plt.close()
        ...
        for k_ind, v_ind in enumerate(v_inds):
            ...
            correct_so_far = np.maximum(np.sum(np.sign(dmdcoeff[:i+1,k_ind]) == np.sign(gnd_truth[:i+1])), np.sum(np.sign(dmdcoeff[:i+1,k_ind]) == -np.sign(gnd_truth[:i+1])))#/(i+1) * 100.
        print(' We have got ' + str(correct_so_far) + '/' + str(i-start+1) + ' correct')
